...
import { usePatientRecordDetail } from "../hooks/usePatientRecordDetail";

const PatientRecordDetail = ({ patient, onBack }) => {
  const dispatch = useDispatch();
  const { patientInfo, visits, isLoading } = usePatientRecordDetail(patient);
                ...
                <div className="pr-4">
                  ...
                  <div className="mb-4 border-b theme-border pb-3">
                    ...
                    <p className="text-sm theme-text-muted mb-1">
                      التخصص: {visit.formattedSpecialization}
                    </p>
                    <p className="text-sm theme-text-muted">
                      📍 {visit.formattedClinic}
                    </p>
                  </div>

                  {/* تفاصيل التشخيص والعلاج */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div className="p-4 rounded-2xl theme-surface-80 border theme-border">
                      <p className="text-xs font-bold theme-text-muted mb-1.5">التشخيص الطبي</p>
                      <p className="text-sm theme-text leading-relaxed">{visit.diagnosis}</p>
                    </div>
                    <div className="p-4 rounded-2xl theme-surface-80 border theme-border">
                      <p className="text-xs font-bold theme-text-muted mb-1.5">الوصفة الطبية (الروشتة)</p>
                      <p className="text-sm theme-text leading-relaxed">{visit.prescription}</p>
                    </div>
                  </div>

                  {/* بيانات وقت الزيارة ومصدر الحجز */}
